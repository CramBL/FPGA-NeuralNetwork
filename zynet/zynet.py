...
class model:
    ...
    def compile(
        ...
        if pretrained == "Yes":
            genWegitsAndBias.genWegitsAndBias(
                dataWidth,
                dataWidth - weightIntSize,
                dataWidth - weightIntSize - inputIntSize,
                weights,
                biases,
            )
...
def makeSystem(projectName, blockName):
    xilinxUtils.makeSystem(
        projectName + "/" + projectName + ".xpr", projectName + "/../src/", blockName
    )
